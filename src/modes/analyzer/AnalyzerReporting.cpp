...
constexpr AnalyzerFieldDescriptor kSourceTrialMissReasonField{"source.freq", "trial_miss_reason"};
constexpr AnalyzerFieldDescriptor kSourceFmRejectReasonField{"source.freq", "fm_reject_reason"};
constexpr AnalyzerFieldDescriptor kSourceFmNoEmitReasonField{"source.freq", "fm_no_emit_reason"};
constexpr AnalyzerFieldDescriptor kSourceFmGateReasonField{"source.freq", "fm_gate_reason"};
...
void AnalyzerApp::printSequenceInspect(const AnalyzerReport& report) const {
    ...
    if (report.profileDetail.inspectionModuleCount > 1 &&
        ...
        Serial.print(static_cast<unsigned long>(report.profileDetail.scalarObservation.preFloorValueCount));
        Serial.print(" scalar.pre_floor_bucket_count=");
        Serial.print(static_cast<unsigned long>(report.profileDetail.scalarObservation.preFloorBucketCount));
        Serial.print(" scalar.pre_floor_covered_ms=");
        Serial.print(static_cast<unsigned long>(report.profileDetail.scalarObservation.preFloorCoveredMs));
        Serial.print(" scalar.pre_floor_coverage_ratio=");
        Serial.print(report.profileDetail.scalarObservation.preFloorCoverageRatio, 3);
        Serial.print(" scalar.pre_floor_median=");
        Serial.print(report.profileDetail.scalarObservation.preFloorMedian, 1);
        Serial.print(" scalar.pre_floor_p75=");
        Serial.print(report.profileDetail.scalarObservation.preFloorP75, 1);
        Serial.print(" scalar.pre_floor_rms=");
        Serial.print(report.profileDetail.scalarObservation.preFloorRms, 1);
    ...
    if (strcmp(supportTarget, "FrequencyScoreStrength") == 0) {
        ...
        Serial.print(report.profileDetail.scalarObservation.coverageRatio, 3);
        Serial.print(" evidence.scalar.pre_floor_window_start_ms=");
        Serial.print(report.profileDetail.scalarObservation.preFloorWindowStartMs);
        ...
        Serial.print(static_cast<unsigned long>(report.profileDetail.scalarObservation.preFloorValueCount));
        Serial.print(" evidence.scalar.pre_floor_bucket_count=");
        Serial.print(static_cast<unsigned long>(report.profileDetail.scalarObservation.preFloorBucketCount));
        Serial.print(" evidence.scalar.pre_floor_covered_ms=");
        Serial.print(static_cast<unsigned long>(report.profileDetail.scalarObservation.preFloorCoveredMs));
        Serial.print(" evidence.scalar.pre_floor_coverage_ratio=");
        Serial.print(report.profileDetail.scalarObservation.preFloorCoverageRatio, 3);
        Serial.print(" evidence.scalar.pre_floor_median=");
        Serial.print(report.profileDetail.scalarObservation.preFloorMedian, 1);
        Serial.print(" evidence.scalar.pre_floor_p75=");
        Serial.print(report.profileDetail.scalarObservation.preFloorP75, 1);
        Serial.print(" evidence.scalar.pre_floor_rms=");
        Serial.print(report.profileDetail.scalarObservation.preFloorRms, 1);
...
void AnalyzerApp::printSequenceDiagnostics(const AnalyzerReport& report) const {
    ...
    if (compactSource) {
        ...
        if (report.frequency.acceptedPresent) {
            Serial.print(' ');
            printField(kSourceAcceptedDtMsField, report.frequency.acceptedDtMs);
            Serial.print(' ');
            printField(kSourceAcceptedDurationMsField, report.frequency.acceptedDurationMs);
            Serial.print(' ');
            printField(kSourceAcceptedStrengthField, report.frequency.acceptedStrength, 1);
        }
        Serial.println();
        if (report.frequency.sourceOccurrenceEmitted && !report.frequency.acceptedPresent) {
            Serial.print("SEQ_SOURCE_REJECT");
            Serial.print(' ');
            printField(kSourceSelectedRejectPeakMsField, report.frequency.fmPeakMs);
            Serial.print(' ');
            printField(kSourceSelectedRejectDurationMsField, report.frequency.fmDurationMs);
            Serial.print(' ');
            ...
            Serial.print(' ');
            printField(kSourceFmRejectReasonField, report.frequency.fmRejectReason != nullptr ? report.frequency.fmRejectReason : "unknown");
            Serial.print(' ');
            printField(kSourceFmNoEmitReasonField, report.frequency.fmNoEmitReason != nullptr ? report.frequency.fmNoEmitReason : "none");
            Serial.print(' ');
            printField(kSourceFmGateReasonField, report.frequency.fmGateReason != nullptr ? report.frequency.fmGateReason : "none");
            Serial.println();
        }
        return;
    }
    ...
    if (report.frequency.acceptedPresent) {
        ...
        printField(kSourceAcceptedStrengthField, report.frequency.acceptedStrength, 1);
        Serial.print(' ');
        printField(kSourceAcceptedScoreField, report.frequency.acceptedScore, 1);
        Serial.print(' ');
        printField(kSourceAcceptedContrastField, report.frequency.acceptedContrast, 2);
    }
    Serial.print(" ");
    ...
    Serial.print(' ');
    printField(kSourceDiagFirstFrameField, report.frequency.diagFirstFrameMs);
    Serial.print(' ');
    printField(kSourceDiagLastFrameField, report.frequency.diagLastFrameMs);
    Serial.print(' ');
    printField(kSourceExpectedWindowField, report.frequency.expectedWindowMs);
    Serial.print(' ');
    printField(kSourceExpectedFrameCountField, report.frequency.expectedFrameCountEstimate);
    Serial.print(' ');
    ...
    Serial.print(' ');
    printField(kSourceMeanScoreField, report.frequency.meanScore, 1);
    Serial.print(' ');
    printField(kSourceMeanContrastField, report.frequency.meanContrast, 2);
    Serial.print(' ');
    printField(kSourceScoreThresholdField, report.frequency.scoreThreshold, 1);
    Serial.print(' ');
    printField(kSourceContrastThresholdField, report.frequency.contrastThreshold, 2);
    Serial.print(' ');
    printField(kSourceMaxScoreField, report.frequency.maxScore, 1);
    Serial.print(' ');
    printField(kSourceMaxScoreMsField, report.frequency.maxScoreMs);
    Serial.print(' ');
    printField(kSourceMaxContrastField, report.frequency.maxContrast, 2);
    Serial.print(' ');
    printField(kSourceMaxContrastMsField, report.frequency.maxContrastMs);
    Serial.print(' ');
    printField(kSourceTrialMissReasonField, report.frequency.trialMissReason != nullptr ? report.frequency.trialMissReason : "unknown");
    Serial.print(' ');
    ...
    if (report.frequency.sourceOccurrenceEmitted && !report.frequency.acceptedPresent) {
        ...
        printField(kSourceSelectedRejectDurationMsField, report.frequency.fmDurationMs);
        Serial.print(' ');
        printField(kSourceSelectedRejectWindowSamplesField, report.frequency.peakWindowSampleCount);
        Serial.print(' ');
        printField(kSourceFreqPeakScoreField, report.frequency.peakScore, 1);
        Serial.print(' ');
        printField(kSourceFreqPeakContrastField, report.frequency.peakContrast, 2);
    ...
    Serial.print(' ');
    printField(kSourceCurrentTrialIdField, report.frequency.currentTrialId);
    if (report.frequency.inconsistent) {
        Serial.print(' ');
        printField(kSourceAcceptedTrialIdField, report.frequency.acceptedTrialId);
        Serial.print(' ');
        printField(kSourceAcceptedSourceField, report.frequency.acceptedSource != nullptr ? report.frequency.acceptedSource : "none");
        Serial.print(' ');
    ...
    Serial.print(' ');
    printField(kSourceLiveFreqReasonField, report.frequency.liveFreqReason != nullptr ? report.frequency.liveFreqReason : "none");
    Serial.print(' ');
    printField(kSourceLiveFreqWouldField, report.frequency.liveFreqWould != nullptr ? report.frequency.liveFreqWould : "none");
    Serial.print(' ');
    printField(kSourceLiveFreqReadyField, report.frequency.liveFreqReady);
    Serial.print(' ');
    printField(kSourceLiveFreqGateField, report.frequency.liveFreqGate);
    Serial.print(' ');
    printField(kSourceLiveFreqPresentField, report.frequency.liveFreqPresent);
    Serial.print(' ');
    printField(kSourceLiveFreqValidField, report.frequency.liveFreqValid);
    Serial.print(' ');
    printField(kSourceLiveFreqMatchField, report.frequency.liveFreqMatch);
    Serial.print(' ');
...
void AnalyzerApp::printSequenceScalarDiagnostics(const AnalyzerReport& report) const {
    ...
    if (report.scalar.acceptedPresent) {
        ...
        Serial.print(' ');
        printField(kScalarAcceptedStartMsField, report.scalar.acceptedStartMs);
        Serial.print(' ');
        printField(kScalarAcceptedPeakMsField, report.scalar.acceptedPeakMs);
        Serial.print(' ');
        printField(kScalarAcceptedReleaseMsField, report.scalar.acceptedReleaseMs);
        Serial.print(' ');
        ...
        Serial.print(' ');
        printField(kScalarBestPeakMsField, report.scalar.scalarPeakMs);
        Serial.print(' ');
        printField(kScalarBestDurMsField, report.scalar.scalarDurationMs);
        Serial.print(' ');
        printField(kScalarBestStrengthField, report.scalar.scalarPeakStrength, 1);
    }
    Serial.print(' ');
